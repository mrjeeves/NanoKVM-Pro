...
- **CEC hand raise** (`server/service/mesh/cec.go`) — the KVM can raise a hand on the [CEC Support](https://github.com/mrjeeves/CECSupport) help queue (a `SupportPresence` beacon on the `cecsupport-clients` mesh, exactly like a CEC customer), so a technician sees the device needs help along with its 9-digit support number. Raise/lower from the web UI's Mesh tab or the `/api/mesh/help/*` endpoints. A physical **USR-button** trigger (double short-press, `server/service/button/`) also ships but is **off by default**: unlike the PCIe board's BOOT button, the Pro's USR button isn't surfaced to Linux by this firmware, so confirm its evdev node on the device, set `mesh.handRaise.inputDevice`, and set `mesh.handRaise.buttonEnabled: true` in `server.yaml` to turn it on.
